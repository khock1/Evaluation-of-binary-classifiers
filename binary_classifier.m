...
% simple way to create dummy data; use the true condtion sequence, and
% switch values with some random chance to obtain valus for a 'predicted condition'; 
% the less values switch, the more like true condition this sequence will be
...
% loop through the samples of predicted condtions to evaualte them
for p = 1:n_predictions
    classifier(p).prediction = p;% record the number of samples
    confus_mat = zeros(2);% confusion matrix
    for m = 1:n_measures% now, determine the match between true condition and predicted condition and build a confusion matrix out of that
        this_x = x(1, m);
        this_y = y(p, m);
        if this_x == 1
            if this_y == 1
    ...
    % spell out the true and false positives and negatives for easier tracking
    ...
    % names of most statistics are self-explanatory or easily found in literature or Wikipedia
...
xlabel('Prediction No');
ylabel('Accuracy');
% find all predictions that perfomed above the threshold
